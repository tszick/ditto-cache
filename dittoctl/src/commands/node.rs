use crate::{client::{enc, mgmt_get, mgmt_post}, config::CtlConfig};
use anyhow::Result;
use clap::Subcommand;
...
pub async fn run(cmd: NodeCommand, cfg: &mut CtlConfig, client: &reqwest::Client) -> Result<()> {
    ...
    match cmd {
        ...
        NodeCommand::Get { property, target } => {
            let url  = format!("{}/api/nodes/{}/property/{}", base, enc(&target), enc(&property));
            let data = mgmt_get(client, &url).await?;
        ...
        NodeCommand::Set { property, target, value } => {
        ...
        NodeCommand::Status { target } => {
            ...
            for node in nodes {
                ...
                println!("  {:<22} {}", "committed-index",  node["committed_index"].as_u64().map(|v| v.to_string()).as_deref().unwrap_or("?"));
