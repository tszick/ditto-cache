...
- **Admin authentication** — HTTP Basic Auth on node REST (7778), plus Basic or Bearer/OIDC introspection on the management port (7781)
...
| `DITTO_BACKUP_ENCRYPTION_KEY_ENV` | `backup.encryption_key_env` |
| `DITTO_BACKUP_ENCRYPTION_KEY_FILE` | `backup.encryption_key_file` |
...
# bearer_introspection_client_secret = "<client-secret>" # legacy/local-dev only
...
# password = "<node-http-password>" # legacy/local-dev only
```
...
# password = "<mgmt-password>" # legacy/local-dev only
# Or Bearer:
...
- `ditto-mgmt` refuses startup without mTLS, HTTPS cert/key, and either Basic or Bearer admin auth.
...
Port 7778 (HTTP REST) can run HTTPS plus HTTP Basic Auth. Port 7781
(`ditto-mgmt`) can run HTTPS plus Basic or Bearer/OIDC introspection auth.
In production, keep strict mode enabled.
...
**Backup encryption** is separate from TLS: set `backup.encryption_key_env`,
`backup.encryption_key_file`, or `DITTO_BACKUP_ENCRYPTION_KEY` (hex-encoded
32-byte key) to encrypt backup files with AES-256-GCM. The key is independent
of the TLS certificates. Generate a key with:
